...
*   **Conda** with the following Bioconductor packages installed in your environment:
    ```bash
...
Extract the coordinates for any sequence with a `HitCount > 0`. The output file (`hit_coords.txt`) contains one `start-end` coordinate pair per line and can grow to tens of thousands of entries for a genome-wide search.
...
> **Note:** This script assumes the required R packages (`clusterProfiler`, `org.Hs.eg.db`) are already installed in your active Conda environment. It will not attempt to install them automatically and will exit with an informative error if they are missing.
